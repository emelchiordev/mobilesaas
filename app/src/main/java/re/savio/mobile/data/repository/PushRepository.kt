...
class PushRepository @Inject constructor(
    ...
    private fun buildColdMeasurePayload(measure: ColdMeasureEntity): Map<String, Any?> =
        buildMap {
            ...
            put("qtefri3", measure.qtefri3.toDoubleOrNull())
            ...
            put("supplyContainerId", measure.supplyContainerId.takeIf { it.isNotBlank() })
            put("recoveryContainerId", measure.recoveryContainerId.takeIf { it.isNotBlank() })
    ...
    private suspend fun restoreOperationalSyncStatusAfterVersionResync(
        row: re.savio.mobile.data.local.entity.InterventionEntity,
    ) {
        val operational =
        ...
        results.forEach { result ->
            ...
            when {
                result.status == "rejected" && result.reason == "IMMUTABLE" -> {
                    ...
                    re.savio.mobile.observability.SavioSyncSentry.onConflictImmutable(
                        interventionId,
                        attempts,
                    )
                    android.util.Log.w(
                ...
                result.status == "conflict" &&
                    ...
                    re.savio.mobile.observability.SavioSyncSentry.onConflictVersion(
                        interventionId,
                        clientVersion,
                    )
                    android.util.Log.w(
