package re.savio.mobile.ui.screen.invoice
...
import re.savio.mobile.data.remote.dto.PrestationDto
import re.savio.mobile.domain.invoice.InvoiceNatureLigne
...
fun InvoiceScreen(
    ...
    val natureValidationLineIds by viewModel.natureValidationLineIds.collectAsStateWithLifecycle()
    val canEditVatRate by viewModel.canEditVatRate.collectAsStateWithLifecycle()
        ...
        if (isLoading) {
            ...
            LazyColumn(
                ...
                items(sortedLines, key = { it.id }) { line ->
                    if (isEditable) {
                        // Surcharge top-level (state=) : évite animateItemPlacement retiré en Compose 1.7+
                        ReorderableItem(
                            state = reorderState,
                            key = line.id,
                            ...
                            InvoiceLineRow(
                                ...
                                canEdit = true,
                                canEditVatRate = canEditVatRate,
                                natureHasError = line.id in natureValidationLineIds,
                                onNatureChange = { viewModel.updateNatureLigne(line.id, it) },
    ...
    if (showAddFree) {
        AddFreeLineDialog(
            canEditVatRate = canEditVatRate,
            onConfirm = { reference, label, qty, price, vat, billingType, natureLigne ->
                viewModel.addFreeLine(reference, label, qty, price, vat, billingType, natureLigne)
                showAddFree = false
            },
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun NatureLigneSelector(
    selectedCode: String?,
    ...
    val selectedLabel =
        InvoiceNatureLigne.labelFor(selectedCode) ?: "Nature comptable *"
    val borderColor =
    ...
    ExposedDropdownMenuBox(
        ...
        ExposedDropdownMenu(
            ...
            InvoiceNatureLigne.OPTIONS.forEach { option ->
                DropdownMenuItem(
...
private fun InvoiceLineRow(
    ...
    natureHasError: Boolean,
    onNatureChange: (String) -> Unit,
    onDelete: () -> Unit,
) {
...
fun InvoiceLineCard(
    ...
    canEdit: Boolean,
    canEditVatRate: Boolean = true,
    natureHasError: Boolean = false,
    onNatureChange: (String) -> Unit = {},
    onDelete: () -> Unit,
) {
    val showNatureEditor = canEdit && InvoiceNatureLigne.canEditNature(line)
    val showNatureReadOnly =
        !canEdit && InvoiceNatureLigne.isFreeLine(line) && !line.natureLigne.isNullOrBlank()
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(
            containerColor = if (isDragging) {
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                if (showNatureEditor) {
                    ...
                    NatureLigneSelector(
                        selectedCode = line.natureLigne,
                        onSelected = onNatureChange,
                        hasError = natureHasError,
                    )
                } else if (showNatureReadOnly) {
                    Text(
                        InvoiceNatureLigne.labelFor(line.natureLigne)?.let { "Nature : $it" }
                            ?: line.natureLigne.orEmpty(),
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                }
                ...
                Text(
                    if (canEditVatRate) {
                        ...
                        "%.0f × %.2f € HT".format(line.quantity, line.unitPriceHt)
                    },
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
                if (InvoiceNatureLigne.isPhotoEligible(line)) {
                    Spacer(Modifier.height(6.dp))
                    Row(
                        verticalAlignment = Alignment.CenterVertically,
                        horizontalArrangement = Arrangement.spacedBy(8.dp),
                    ) {
                        if (!line.photoThumbnailUrl.isNullOrBlank()) {
                            AsyncImage(
                                model = line.photoThumbnailUrl,
                        ...
                        Text(
                            when {
                                line.photoIncluded && line.hasEffectivePhoto ->
                                    if (line.photoSource == "line") "Photo incluse (spécifique)"
                                    else "Photo incluse (catalogue)"
                                line.hasEffectivePhoto -> "Photo disponible"
                                else -> "Photo produit (Pièces)"
                            },
                            style = MaterialTheme.typography.labelSmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            ExposedDropdownMenuBox(
                ...
                onExpandedChange = { if (canEditVatRate) vatExpanded = !vatExpanded },
            ) {
                OutlinedTextField(
                    value = if (canEditVatRate) "$vat %" else "—",
                    onValueChange = {},
                    readOnly = true,
                    enabled = canEditVatRate,
                    label = { Text("TVA") },
                    trailingIcon = {
                        if (canEditVatRate) {
            ...
            Button(
                onClick = {
                    onConfirm(
                        ...
                        if (canEditVatRate) parseDecimalInput(vat) ?: item.vatRate else 0.0,
                        billingType,
                    )
                },
                modifier = Modifier.fillMaxWidth(),
...
fun AddFreeLineDialog(
    ...
    var billingType by remember { mutableStateOf("billable") }
    var natureLigne by remember { mutableStateOf<String?>(null) }
    var natureTouched by remember { mutableStateOf(false) }
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                NatureLigneSelector(
                    selectedCode = natureLigne,
                    onSelected = {
                        natureLigne = it
                        natureTouched = true
                    },
                    hasError = natureTouched && natureLigne.isNullOrBlank(),
                )
            }
        },
        confirmButton = {
            TextButton(
                onClick = {
                    natureTouched = true
                    val selectedNature = natureLigne ?: return@TextButton
                    onConfirm(
                        reference.trim().ifBlank { null },
                        label.trim(),
                        parseDecimalInput(qty) ?: 1.0,
                        parseDecimalInput(price) ?: 0.0,
                        if (canEditVatRate) parseDecimalInput(vat) ?: 20.0 else 0.0,
                        billingType,
                        selectedNature,
                    )
                },
...
@Composable
fun InvoiceSignaturesBlock(invoice: re.savio.mobile.data.local.entity.InvoiceEntity) {
    Text(
        "Signatures",
        style = MaterialTheme.typography.titleSmall,
        fontWeight = FontWeight.SemiBold,
