...
import re.savio.mobile.ui.refonte.SavioClotureDashedAddButton
...
private fun AnomalyDraftRow(
    ...
    val refonte = useSavioRefonteUi()
    if (refonte) {
        SavioRefonteCard(modifier = Modifier.fillMaxWidth(), contentPadding = false) {
            Row(
                modifier =
                    Modifier
                        .fillMaxWidth()
...
fun ClosureDgiSummaryBanner(dgiCount: Int) {
    ...
    val refonte = useSavioRefonteUi()
    val text =
        "⚠ $dgiCount anomalie${if (dgiCount > 1) "s" else ""} DGI — client informé"
    if (refonte) {
        SavioClotureNoticeCard(text = text, isError = true)
        return
    }
    Surface(
        modifier = Modifier.fillMaxWidth(),
