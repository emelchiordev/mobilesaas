...
import re.savio.mobile.ActivationDeepLink
...
import re.savio.mobile.data.remote.interceptor.AuthEvent
import re.savio.mobile.data.remote.interceptor.AuthEventBus
import re.savio.mobile.ui.screen.auth.AccountPrepScreen
import re.savio.mobile.ui.screen.auth.ActivationScreen
import re.savio.mobile.ui.screen.auth.AuthViewModel
import re.savio.mobile.ui.screen.auth.LoginScreen
import re.savio.mobile.ui.screen.auth.OnboardingScreen
import re.savio.mobile.ui.screen.auth.RegisterScreen
import re.savio.mobile.ui.screen.auth.SelectSocieteScreen
import re.savio.mobile.ui.screen.auth.SplashScreen
import re.savio.mobile.ui.screen.auth.WelcomeScreen
import re.savio.mobile.ui.screen.client.CreateClientScreen
import re.savio.mobile.ui.screen.intervention.create.CreateInterventionScreen
import re.savio.mobile.ui.screen.intervention.offline.CreateOfflineInterventionScreen
import re.savio.mobile.ui.utils.NetworkUtils
import androidx.compose.ui.platform.LocalContext
import re.savio.mobile.ui.screen.intervention.offline.PendingOfflineInterventionsScreen
import re.savio.mobile.ui.screen.intervention.CameraScreen
import re.savio.mobile.ui.screen.intervention.ClientDetailScreen
import re.savio.mobile.ui.screen.intervention.EquipementDetailScreen
import re.savio.mobile.ui.screen.intervention.EquipementDetailViewModel
import re.savio.mobile.ui.screen.intervention.cerfa.CerfaFroidScreen
import re.savio.mobile.ui.screen.intervention.cerfa.CerfaScreen
import re.savio.mobile.ui.screen.intervention.CatalogSearchScreen
import re.savio.mobile.ui.screen.intervention.EquipmentFormScreen
import re.savio.mobile.ui.screen.intervention.PlateScanScreen
import re.savio.mobile.ui.screen.intervention.attestation.AttestationVeScreen
import re.savio.mobile.ui.screen.intervention.measure.MeasureScreen
import re.savio.mobile.ui.screen.intervention.pacmeasure.PacMeasureScreen
import re.savio.mobile.ui.screen.intervention.InterventionActiveScreen
import re.savio.mobile.ui.screen.intervention.InterventionDetailScreen
import re.savio.mobile.ui.screen.invoice.DevisSignatureScreen
import re.savio.mobile.ui.screen.invoice.InvoiceScreen
import re.savio.mobile.ui.screen.intervention.PhotosScreen
import re.savio.mobile.ui.screen.intervention.cloture.ClotureRapportScreen
import re.savio.mobile.ui.screen.intervention.cloture.ClotureSignatureScreen
import re.savio.mobile.ui.navigation.MainShellScreen
import re.savio.mobile.ui.screen.tournee.TourneeViewModel
import re.savio.mobile.ui.viewmodel.PhotoViewModel
...
fun AppNavigation(
    ...
    val authSnackbarHostState = remember { SnackbarHostState() }

    val navigateAccountPrep: () -> Unit = {
        navController.navigate(Screen.AccountPrep.route) {
    ...
    val logoutAndGoWelcome: () -> Unit = {
        authViewModel.logout(
            ...
            onNeedsAccountPrep = navigateAccountPrep,
        )
    }

    LaunchedEffect(authUiState.logoutBlockedMessage) {
        authUiState.logoutBlockedMessage?.let {
            authSnackbarHostState.showSnackbar(it)
        ...
        authEventBus.events.collect { event ->
            when (event) {
                is AuthEvent.Unauthorized -> {
                    // AccountPrep owns the handoff; a 401 during flush must not bounce to Welcome.
                    if (
                        accountSwitchCoordinator.readSwitchState().state !=
                            AccountSwitchStateStore.State.IDLE
                    ) {
                        return@collect
                    }
                    navController.navigate(Screen.Welcome.route) {
    ...
    NavHost(
        ...
        composable(Screen.AccountPrep.route) {
            AccountPrepScreen(
                onGoMain = {
                ...
                onStayOnOutgoingMain = { message ->
                    navController.navigate(Screen.Main.route) {
                        popUpTo(navController.graph.id) { inclusive = true }
                        launchSingleTop = true
                    }
                    scope.launch { authSnackbarHostState.showSnackbar(message) }
                },
                onReauthWelcome = { message ->
                    navController.navigate(Screen.Welcome.route) {
                        popUpTo(navController.graph.id) { inclusive = true }
                        launchSingleTop = true
                    }
        ...
        composable(Screen.Register.route) {
            RegisterScreen(
                ...
                onNavigateAccountPrep = navigateAccountPrep,
                onNavigateToLogin = {
                    navController.navigate(Screen.Login.route) {
                        popUpTo(Screen.Register.route) { inclusive = true }
                        launchSingleTop = true
        ...
        composable(Screen.Main.route) { backStackEntry ->
            ...
            val pendingShowDemoCompletionBanner by backStackEntry.savedStateHandle
                .getStateFlow("pending_show_demo_completion_banner", false)
            ...
            MainShellScreen(
                ...
                pendingShowDemoCompletionBanner = pendingShowDemoCompletionBanner,
                onConsumePendingShowDemoCompletionBanner = {
                    backStackEntry.savedStateHandle["pending_show_demo_completion_banner"] = false
                },
        ...
        composable(
            ...
            ClotureSignatureScreen(
                ...
                onCompleted = { wasDemo ->
                    if (wasDemo) {
                        navController.getBackStackEntry(Screen.Main.route).savedStateHandle
                            .set("pending_show_demo_completion_banner", true)
                    }
                    navController.navigate(Screen.Main.route) {
                        popUpTo(Screen.Main.route) { inclusive = false }
                        launchSingleTop = true
        ...
        composable(
            ...
            val parentEquipmentId = backStackEntry.arguments?.getString("parentEquipmentId")
                ?.takeIf { it.isNotBlank() }
            val formViewModel: re.savio.mobile.ui.screen.intervention.EquipmentFormViewModel =
